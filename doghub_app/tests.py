...
class HomeViewTestCase(TestCase):
    ...
    def test_home_view(self):
        ...
        self.assertContains(response, "Welcome")  # Check if the response contains "DogHub"
...
class ResgisterViewTestCase(TestCase):
    ...
    def test_register_request_valid(self):
        # Ensure that a POST request with valid data creates a new user
        data = {
    ...
    def test_register_request_existing_user(self):
        # Ensure that a POST request with an existing user email displays an error message
...
class LoginTestCase(TestCase):
    ...
    def test_login_success(self):
        ...
        self.assertEqual(response.status_code,302)
        self.assertTrue(CustomUser.objects.filter(email=self.user_data['uemail']).exists())
    ...
    def test_login_failure(self):
        ...
        response = self.client.post(self.login_url, data=self.user_data)
        self.assertEqual(response.status_code,200)
        self.assertFalse(CustomUser.objects.filter(email=self.user_data['uemail']).exists())